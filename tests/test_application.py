    ...
    async with TestKafkaBroker(broker):
        async with TestApp(app):
            ...
            on_new_crypto_price.mock.assert_called_with(
                dict(CryptoPrice(price=50000.0, crypto_currency="BTC"))
            )
            ...
            on_new_crypto_price.mock.assert_called_with(
                dict(CryptoPrice(price=60000.0, crypto_currency="BTC"))
            )
            on_price_mean.mock.assert_not_called()
            ...
            on_new_crypto_price.mock.assert_called_with(
                dict(CryptoPrice(price=70000.0, crypto_currency="BTC"))
            )
